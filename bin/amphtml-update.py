...
def GenValidatorPb2Py(validator_directory, out_dir):
	...
	subprocess.check_call(
		[
			'protoc',
			os.path.join(validator_directory, 'validator.proto'),
			f'--proto_path={validator_directory}',
...
def GeneratePHP(repo_directory, out_dir):
	...
	for expected_spec_name in expected_spec_names:
		if expected_spec_name not in seen_spec_names:
			raise Exception(f'Missing spec: {expected_spec_name}')
...
def GenerateHeaderPHP(out):
	...
	out.append('<?php')
	out.append('/**')
	out.append(f' * Generated by {os.path.basename(__file__)} - do not edit.')
	out.append(' *')
